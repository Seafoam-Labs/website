...
to maintain. See the [layout.toml reference](/aqueous/docs/toml/layout/) for
the full list of accepted keys. The sidecar overlays `wm.toml` one recognized
key at a time, so an omitted value keeps the setting already loaded from
`wm.toml` or the compiled default.
