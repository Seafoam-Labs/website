...
Aqueous keeps its configuration in four small TOML files. [`wm.toml`](/aqueous/docs/toml/wm/)
is the entry point; the other files are optional overlays that keep layout,
input, and window policy easy to navigate.
...
adaptive_sync = true
# Enable only on capable DRM outputs; uses the fixed HDR10 profile.
hdr = true
layout = "tile"
...
connector names are not stable. Run `aqueousctl outputs` to see connector
names, supported modes, and the current/preferred mode before writing a display
block.
...
variants. `scale` must be between `0.5` and `3.0`. Set `hdr = true` only for a
capable DRM output; Aqueous uses a fixed HDR10 profile (10-bit BT.2020 with PQ).
