...
4. ### Install the generated package
   ...
   :::tip[Or let Shelly install it for you]
   Pass `-l` (or `--install`) to install the built archives as soon as the build succeeds, like `makepkg -i`. See
...
`-l` (`--install`) installs the archives the build just produced, so a package no longer has to be built and then
installed by hand:

```sh
shelly build --install
# Equivalent to makepkg -si
shelly build -s --install
shelly -Asl
```

Like `makepkg -i`, only the freshly built archives are added, so combine the flag with `-s` when the package's own
dependencies may be missing.

Administrator credentials are requested **once, up front** — the builder locks itself against elevation with
`PR_SET_NO_NEW_PRIVS` and can never elevate afterwards. The build runs unprivileged as your regular user, and the
elevated coordinator installs the archives afterwards, through the same path as `shelly install standard <archive>`.

Other details worth knowing:

- With `-s -l`, build-dependency cleanup runs _after_ the install, so a runtime dependency the built package now needs
  is not removed.
- Split package members are installed in one transaction, so they can satisfy each other's dependencies.
- With `-i`, the guest exports the archives and the host coordinator installs them; the container never writes to the
  host database.
- Combining it with `--json`, `--ui-mode`, `--review-only`, or `--makesrcinfo` exits `2` with
  `Cannot combine --install with <mode>.` before building.

:::caution[Short options do not cluster]
`-l` is the install alias; `-i` remains `--isolated`. So `shelly build -si` is rejected — write `shelly build -s -l` for
sync-then-install and `shelly build -s -i` for a sync-then-isolated build. The [shortcode](/shelly-alpm/docs/cli-reference/#shortcodes)
form does combine: `-Asl` means `shelly build -s -l`, while `-Asi` is the isolated build.
:::
...
| `--install`                   | `-l`  | Install the built package archives after a successful build. With `-s` this mirrors `makepkg -si`.                   |
...
Adding `-l` (`--install`) is safe here: the guest exports the archives and the host coordinator installs them, so the
container never writes into the host system database.
