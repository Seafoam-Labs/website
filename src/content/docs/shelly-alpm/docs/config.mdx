...
If you set the environment variable `SHELLY_ELEVATOR` then Shelly will use the designated privlege elevator in the cli rather than detecting the elevator in path. Shelly will fall back to sudo if nothing is detected.
The natively detected elevators are currently: `doas` and `sudo`
